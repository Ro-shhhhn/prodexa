// backend/controllers/productController.js - FIXED VERSION with Partial Search
const Product = require('../models/Product');
const Category = require('../models/Category');
const SubCategory = require('../models/SubCategory');
const mongoose = require('mongoose');

// Get products with PARTIAL SEARCH functionality - MAIN FIX
// backend/controllers/productController.js - FIXED VERSION with Multiple Subcategory Filter
const getProducts = async (req, res) => {
  try {
    ...
    // FIXED: Handle multiple subcategories
    if (subcategories && subcategories.trim()) {
      const subcategoryIds = subcategories.split(',').filter(id => id.trim());
...
const createProduct = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to create product.please make sure the product  name is valid',
      error: error.message
    });
  }
};
